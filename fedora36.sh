...
# Install and enable useful repositories
sudo dnf -y install vim fedora-workstation-repositories \
...
sudo dnf install -y nemo* la-capitaine*

# Makes nemo, nemo-autostart and nemo-autorun-software visible in Gnome
sudo sed -i 's/^[^#]*OnlyShowIn=X-Cinnamon;/#&/' /usr/share/applications/nemo.desktop
